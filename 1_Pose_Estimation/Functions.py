...
#canvas = np.copy(oriImg)
 ...
 #       print time.time()-tictic,"first part"
  ...
  #      print 'loop', m ,' ',time.time()-tictic, "second part"
    ...
    abcd = heatmap_avg.tolist()
    json.dump(abcd, codecs.open("heatmap.json", 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)
    #with open("heatmap.json", 'w') as outfile:
     #   json.dump(heatmap_avg, outfile, indent=10)
...
########################################################################################
    ...
    return canvas, output_coordinates
